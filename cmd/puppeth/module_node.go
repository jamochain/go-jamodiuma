...
{{end}}
RUN \
  echo 'gjam --cache 512 init /genesis.json' > gjam.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.jamodium/keystore/ && cp /signer.json /root/.jamodium/keystore/' >> gjam.sh && \{{end}}
	echo $'exec gjam --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gjam.sh

ENTRYPOINT ["/bin/sh", "gjam.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.jamodium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.jamhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.jamodium"],
		ethashdir:  infos.volumes["/root/.jamhash"],
		port:       port,
